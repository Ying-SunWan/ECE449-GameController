...
class GeneticController(KesslerController):
    ...
    def __init__(self, chromosome=[0.04658551433549358, 0.030727093029981722, 0.09668246012219399, -1.0207573055558044, -0.19243920787813976, 0.390137562011035, 0.027858804559162734, 0.9413477167474302, -51.10589555418835, -4.191249649271473, -7.590139599347852, 84.1324567428102, 57.05378578526497, 100.5399144837691, 174.57229347376895, 129.69806752894402, 0.915560729509751, 1.8587003991821485, 3.5456460550449584, 1.0915429418849487, 3.7524107267170863, 3.3495082995328813, 0.9522215216859883, 1.0613739615696856, 1.7986156463001044, 3.6254741699052264, 3.738477630070891, 78.07760912050087, 79.51120337705927, 249.03233798585063]):
    ...
    def setup_target_fuzzy_system(self):
        ...
        bullet_time['S'] = fuzz.trimf(bullet_time.universe, [0, 0, self.chromosome[0]])
        bullet_time['M'] = fuzz.trimf(bullet_time.universe, [0, self.chromosome[1], 0.1])
        bullet_time['L'] = fuzz.smf(bullet_time.universe, self.chromosome[2], 0.1)

        theta_delta['NL'] = fuzz.zmf(theta_delta.universe, -1 * math.pi / 3, self.chromosome[3])
        theta_delta['NS'] = fuzz.trimf(theta_delta.universe, [-1 * math.pi / 3, self.chromosome[4], 0])
        theta_delta['Z'] = fuzz.trimf(theta_delta.universe, [-1 * math.pi / 6, self.chromosome[5], math.pi / 6])
        theta_delta['PS'] = fuzz.trimf(theta_delta.universe, [0, self.chromosome[6], math.pi / 3])
        theta_delta['PL'] = fuzz.smf(theta_delta.universe, self.chromosome[7], math.pi / 3)

        ship_turn['NL'] = fuzz.trimf(ship_turn.universe, [-180, -180, self.chromosome[8]])
        ship_turn['NS'] = fuzz.trimf(ship_turn.universe, [-90, self.chromosome[9], 0])
        ship_turn['Z'] = fuzz.trimf(ship_turn.universe, [-30, self.chromosome[10], 30])
        ship_turn['PS'] = fuzz.trimf(ship_turn.universe, [0, self.chromosome[11], 90])
        ship_turn['PL'] = fuzz.trimf(ship_turn.universe, [self.chromosome[12], 180, 180])
    ...
    def setup_move_fuzzy_system(self):
        ...
        aster_distance['S'] = fuzz.trimf(aster_distance.universe, [0, 0, self.chromosome[13]])
        aster_distance['M'] = fuzz.trimf(aster_distance.universe, [0, self.chromosome[14], 300])
        aster_distance['L'] = fuzz.trimf(aster_distance.universe, [self.chromosome[15], 300, 300])

        asteroid_size['S'] = fuzz.trimf(asteroid_size.universe, [0, 0, self.chromosome[16]])
        asteroid_size['M'] = fuzz.trimf(asteroid_size.universe, [0, self.chromosome[17], 4])
        asteroid_size['L'] = fuzz.trimf(asteroid_size.universe, [self.chromosome[18], 4, 4])
    ...
    def setup_danger_fuzzy_system(self):
        ...
        collision_time['S'] = fuzz.trimf(collision_time.universe, [0, 0, self.chromosome[19]])
        collision_time['M'] = fuzz.trimf(collision_time.universe, [0, self.chromosome[20], 5])
        collision_time['L'] = fuzz.trimf(collision_time.universe, [self.chromosome[21], 5, 5])

        asteroid_size['S'] = fuzz.trimf(asteroid_size.universe, [0, 0, self.chromosome[22]])
        asteroid_size['MS'] = fuzz.trimf(asteroid_size.universe, [0, self.chromosome[23], 2])
        asteroid_size['M'] = fuzz.trimf(asteroid_size.universe, [1, self.chromosome[24], 3])
        asteroid_size['ML'] = fuzz.trimf(asteroid_size.universe, [2, self.chromosome[25], 4])
        asteroid_size['L'] = fuzz.trimf(asteroid_size.universe, [self.chromosome[26], 4, 4])

        aster_distance['S'] = fuzz.trimf(aster_distance.universe, [0, 0, self.chromosome[27]])
        aster_distance['M'] = fuzz.trimf(aster_distance.universe, [0, self.chromosome[28], 300])
        aster_distance['L'] = fuzz.trimf(aster_distance.universe, [self.chromosome[29], 300, 300])
    ...
    def escape(self,ship_state, map_size, n_closest_asteroids):
        ...
        for ast in n_closest_asteroids:
            ...
            angle = (360 + math.atan2(rel_ay, rel_ax) * 180 / pi) % 360
            angles.append(angle)
